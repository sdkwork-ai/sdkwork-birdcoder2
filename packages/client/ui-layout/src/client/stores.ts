...
type LayoutInfo = {
  ...
  /**
   * The frame's *rail selection* — which app-mode entry stays lit in the mode
   * rail. `code` shows the main-panel surface (the Conversation by default);
   * every other id shows that mode's keyed `mode.page`.
   */
  mode: AppModeId
  /**
   * The *center-column surface* when it differs from the rail: the
   * sidebar-launched modules (Pull Request, automation, markets) open as
   * overlays *inside* the code surface, so `mode` stays `code`, the code rail
   * entry keeps its selection, and only the center column shows the module
   * page. Rail-driven modes switch `mode` directly and clear any overlay.
   */
...
export function createLayoutStore(): EngineStoreHandle<LayoutState, LayoutActions>  {
  const handle = defineStore({
    ...
    actions: {
      ...
      // always dismisses any code-surface overlay and returns the main panel
      // to the Conversation (the conversation, a rail mode, or a returned
      // overlay all need the overlay cleared).
      setMode: (d, mode: AppModeId) => {
        d.layoutInfo.mode = mode
      ...
      setPanelMode: (d, mode: AppModeId | undefined) => { d.layoutInfo.panelMode = mode },
    },
  })
  return handle
}
