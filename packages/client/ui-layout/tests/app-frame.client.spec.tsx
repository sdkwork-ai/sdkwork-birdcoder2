...
/* The template delegates the squeeze to the grid (jsdom does no layout, so
   specs read the specified tracks): [mode rail, sidebar px, rightbar growth
   limit]. The centre's protected minimum must accompany an open right track. */
function tracks(frame: HTMLElement): number[] {
  // Four tracks: the fixed mode rail, the sidebar, the center (1fr), the rightbar.
  const match = /^([\d.]+)px ([\d.]+)px minmax\((0|400)px, 1fr\) minmax\(0px, ([\d.]+)px\)$/.exec(frame.style.gridTemplateColumns)
  if (match === null) throw new Error(`unexpected template: ${frame.style.gridTemplateColumns}`)
  const rightbar = Number(match[4])
  if ((match[3] === '400') !== (rightbar > 0)) throw new Error(`centre minimum out of step: ${frame.style.gridTemplateColumns}`)
  return [Number(match[1]), Number(match[2]), rightbar]
}
...
describe('AppFrame', () => {
  ...
  it('renders owner props for the default sidebar and prospective right panel', () => {
    ...
    // The prospective preference is 45% of the post-rail frame.
    expect(rightOwner()).toEqual({ width: 839, viewportWidth: 1920, canShow: true })
  ...
  it('keeps the closed sidebar mounted at its 56px rail without a handle', () => {
    const { frame, instance, sidebarOwner, getByTestId, queryByTestId } = mountFrame()
    act(() => { instance.actions.toggleSidebar() })
    expect(tracks(frame)).toEqual([56, 56, 0])
...
describe('AppFrame normal width concessions', () => {
  it('measures the frame, not the window, before choosing the first-open preference', () => {
    ...
    expect(instance.getSnapshot().layoutInfo.viewportWidth).toBe(944)
    expect(rightOwner()).toEqual({ width: 425, viewportWidth: 1000, canShow: true })
    act(() => { instance.actions.openRightbar(true, false) })
    expect(instance.getSnapshot().layoutInfo.rightbar).toBe(425)
    resize(1920)
    expect(rightOwner().width).toBe(425)
  })
  ...
  it('shrinks the right panel to 300px, drops its track, and only then squeezes center', () => {
    ...
    expect(tracks(frame)).toEqual([56, 420, 839])
    expect(rightOwner()).toEqual({ width: 380, viewportWidth: 1256, canShow: true })
    resize(1176)
    expect(tracks(frame)).toEqual([56, 420, 784])
    resize(1175)
    expect(tracks(frame)).toEqual([56, 420, 0])
    expect(rightOwner()).toEqual({ width: 0, viewportWidth: 1175, canShow: false })
    expect(frame.querySelector('[data-side="rightbar"]')).toBeNull()
    expect(instance.getSnapshot().layoutInfo).toMatchObject({ rightbarShown: true, rightbar: 839 })
  ...
  it('uses the post-collapse left rail to permit a narrow first opening', () => {
    // 856 minus the 56px mode rail is the 800px frame the narrow solve sees.
    frameWidth = 856
    const { frame, instance, rightOwner } = mountFrame()
    act(() => { instance.actions.toggleSidebar() })
    expect(tracks(frame)).toEqual([56, 280, 0])
    expect(rightOwner()).toEqual({ width: 344, viewportWidth: 856, canShow: true })
    act(() => { instance.actions.openRightbar(true, false) })
    expect(tracks(frame)).toEqual([56, 56, 360])
  ...
  it.each([[812, 300, true], [811, 0, false]] as const)('reports eligibility at %ipx', (width, rightbar, canShow) => {
    frameWidth = width
  ...
  it('re-expands a wide-closed sidebar at the default width while narrow', () => {
    ...
    resize(1036)
    act(() => { instance.actions.toggleSidebar() })
    expect(tracks(frame)[1]).toBe(280)
...
describe('AppFrame right panel presentation', () => {
  ...
  it('releases the fullscreen track with the instant marker while clearing fullscreen', () => {
    ...
    expect(tracks(frame)).toEqual([56, 280, 839])
    act(() => { instance.actions.closeRightbar() })
    expect(tracks(frame)).toEqual([56, 280, 0])
  ...
  it('preserves normal tracks through fullscreen and hides the outer resize handle', () => {
    ...
    act(() => { instance.actions.openRightbar(true, false) })
    expect(tracks(frame)).toEqual([56, 280, 839])
    expect(handleFor(frame, 'rightbar').style.left).toBe('1081px')
    act(() => { instance.actions.openRightbar(true, true) })
    expect(tracks(frame)).toEqual([56, 280, 839])
    expect(rightOwner().width).toBe(839)
    expect(frame.dataset.rightbarFullscreen).toBe('true')
    expect(frame.querySelector('[data-side="rightbar"]')).toBeNull()
    act(() => { instance.actions.openRightbar(true, false) })
    expect(tracks(frame)).toEqual([56, 280, 839])
  ...
  it('keeps resolved panel width independent of the requested track', () => {
    ...
    resize(1100)
    expect(tracks(frame)).toEqual([56, 280, 0])
    expect(rightOwner().width).toBe(364)
    drag(handleFor(frame, 'rightbar'), 736, 746)
    expect(instance.getSnapshot().layoutInfo.rightbar).toBe(354)
    expect(rightOwner().width).toBe(354)
...
describe('AppFrame pointer resizing', () => {
  ...
  it('starts a conceded right drag at its actual width, shared by panel and track', () => {
    ...
    // The track carries the ratio-clamped preference (70% of the post-rail
    // frame), while the panel reports the resolved squeeze.
    expect(rightOwner().width).toBe(364)
    expect(tracks(frame)[2]).toBeCloseTo(730.8)
    expect(handle.style.left).toBe('736px')
    drag(handle, 736, 746)
    expect(instance.getSnapshot().layoutInfo.rightbar).toBe(354)
    expect(rightOwner().width).toBe(354)
    expect(tracks(frame)[2]).toBe(354)
    expect(handle.style.left).toBe('746px')
  })
  ...
  it('widens to the 70% limit and shrinks to 300px through pointer input', () => {
    ...
    drag(handleFor(frame, 'rightbar'), 1706, 0)
    expect(rightOwner().width).toBe(2100)
    expect(tracks(frame)[2]).toBe(2100)
  ...
  it.each(['fullscreen', 'close', 'unmount'])('cancels a pending drag on %s', (change) => {
    ...
    act(() => { instance.actions.openRightbar(true, false) })
    const handle = handleFor(frame, 'rightbar')
    pointer(handle, 'pointerdown', 1081)
    pointer(handle, 'pointermove', 1025)
    act(() => {
    ...
    expect(instance.getSnapshot().layoutInfo.rightbar).toBe(839)
    expect(animationFrames.size).toBe(0)
    expect(handle.hasPointerCapture(1)).toBe(false)
...
describe('AppFrame frame measurement lifecycle', () => {
  it('coalesces observer reports and publishes the latest frame measurement', () => {
    ...
    expect(animationFrames.size).toBe(1)
    expect(instance.getSnapshot().layoutInfo.viewportWidth).toBe(1864)
    act(flushFrames)
    expect(instance.getSnapshot().layoutInfo.viewportWidth).toBe(1144)
