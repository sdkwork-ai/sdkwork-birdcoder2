...
describe('app-mode rail state', () => {
  it('starts in the code mode with no code-surface overlay', () => {
    const store = createLayoutStore()
    const state = store.create().getSnapshot()
    expect(state.layoutInfo.mode).toBe('code')
    expect(state.layoutInfo.panelMode).toBeUndefined()
  })
  ...
  it('setMode switches the rail selection, clears the overlay, and returns the main panel to the Conversation', () => {
    ...
    expect(state.getSnapshot().layoutInfo.mode).toBe('video')
    expect(state.getSnapshot().layoutInfo.panelMode).toBeUndefined()
    expect(state.getSnapshot().panelInfo.activePanelId).toBeNull()
  })

  it('setPanelMode toggles the overlay without touching the rail selection', () => {
    const store = createLayoutStore()
    const state = store.create()
    state.actions.setPanelMode('markets')
