...
describe('ui-iam host settings registration', () => {
  it('registers the ui-iam namespace against the real settings-file provider', async () => {
    const home = await mkdtemp(join(tmpdir(), 'dsh-ui-iam-host-'))
    await writeFile(join(home, 'settings.yaml'), 'ui-iam:\n  presentation: page\n  qrLoginEnabled: true\n')
  ...
  it('resolves the schema defaults when the document has no ui-iam section', async () => {
    const home = await mkdtemp(join(tmpdir(), 'dsh-ui-iam-host-'))
    const ctx = new Context()
    await ctx.plugin(FileSettingsProvider, { dshHome: home }).await()
    await ctx.plugin({ apply, inject: ['settings'] }).await()
