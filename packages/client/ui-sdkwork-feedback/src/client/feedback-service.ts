 ...
 * is (re)built lazily from the shared ui-sdkwork-env profile so an environment switch
 * takes effect without reload. Auth tokens flow from the mounted ui-sdkwork-iam
 * controller merged with the ui-sdkwork-env access token through the shared SDKWork
...
export class FeedbackService {
  ...
  private readClient(): AppStoreClient {
    ...
    if (baseUrl.trim() === '') {
      throw new Error('ui-sdkwork-feedback: baseUrl is not configured')
    }
