...
/**
 * Drive rail entry spec: the 44px cell renders the outline glyph while idle,
 * swaps to the filled glyph with the selection chrome while active, and
 * switches the frame mode on click.
 */
...
describe('DriveRailEntry', () => {
  it('renders the outline glyph while idle and switches the frame mode on click', () => {
    ...
    const { container } = render(
      <DriveRailEntry {...standard} mode="drive" active={false} setMode={setMode} t={t} />,
    )
    const button = container.querySelector('button')!
    expect(button.getAttribute('aria-label')).toBe('mode.drive.label')
    ...
    expect(setMode).toHaveBeenCalledWith('drive')
  })

  it('renders the filled glyph with the selection chrome while active', () => {
    const { container } = render(
      <DriveRailEntry {...standard} mode="drive" active={true} setMode={() => {}} t={t} />,
    )
    const button = container.querySelector('button')!
