...
describe('DriveApp', () => {
  ...
  it('fails loud when no host adapter was configured', () => {
    expect(() => render(<DriveApp />)).toThrow('ui-drive: SDKWork host runtime is not configured')
  })
  ...
  it('fails loud when no base URL is configured', () => {
    ...
    try {
      expect(() => render(<DriveApp />)).toThrow('ui-drive: SDKWork base URL is not configured')
    } finally {
      adapter.dispose()
    }
  })
