...
The explorer right-sidebar tab is fork-owned: gestures claim through this package's DOM bus events (`sdkwork:explorer:open-file/-diff/-url`), and the host reveals the pane with `ctx.sidebarRight.openTab('sdkwork-explorer')`, so upstream merges cannot overwrite the fork tab.
