...
The shell owns the chrome and the projections; every piece of content and copy belongs to a registrant. The slot types the shell renders into belong to ui-settings, the settings domain base; only the shell's own contract types live here, because they reference ui-sdkwork-app-modes' slot type and the base layer must depend on no `ui-*` package.
