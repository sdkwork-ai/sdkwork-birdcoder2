...
`dsh-client-ui-settings-general` 是 dsh Web 客户端的设置外壳：Settings 面板从模式栏底部固定的设置席位（`mode.rail.settings`）打开，带紧凑触发控件与模态外壳；导航由各功能贡献的分区构建；首次运行的用户一次只走一个引导步骤。它还注册设置页面上所有不属于单一功能的内容：触发器、标题栏与关闭控件界面框架、「本地配置文件」操作、「通用」分区及其 `settings.general.item` slot，以及 `settings` 字典。归具体功能所有的行（「权限」、「语言」、「外观」）、分区（「模型」）与条件式首次使用引导步骤仍由各自的功能包提供；外壳本身不自带任何引导文案。
...
用户通过模式栏底部固定的 Settings 控件进入外壳；功能插件通过本外壳所投影的 slot 账本贡献自己的页面与引导步骤。外壳渲染模态面板、由 `settings.section` 条目构建的导航，以及每次只挂载一个的引导步骤。
...
外壳拥有界面框架与投影；每段内容与文案都属于某个注册方。外壳渲染进的那些 slot 类型归 ui-settings——设置领域底座——所有；只有外壳自身的契约类型放在这里，因为它们引用 ui-sdkwork-app-modes 的 slot 类型，而底座不得依赖任何 `ui-*` 包。
