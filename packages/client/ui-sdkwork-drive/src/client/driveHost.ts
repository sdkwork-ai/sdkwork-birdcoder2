...
/* jscpd:ignore-start -- the SDKWork host adapter is one deliberate template
   shared with ui-sdkwork-knowledge's knowledgebaseHost.ts: cross-package value imports
...
class DriveHostRuntimeImpl implements DriveHostRuntime {
  ...
  private readBaseUrl(): string {
    ...
    if (baseUrl === '') throw new Error('ui-sdkwork-drive: SDKWork base URL is not configured')
    return baseUrl
  }
...
export function DriveApp(): ReactNode {
  ...
  if (adapter === undefined) {
    throw new Error('ui-sdkwork-drive: SDKWork host runtime is not configured')
  }
