...
const base = clientBundle('@deepseek-ai/dsh-client-ui-generations-assets', ['lib/types/index.js', 'lib/types/invariant.js'])
...
const ASSETS_COMPONENT_PACKAGE = resolvePath(
  SDKWORK_ROOT,
  'apps/sdkwork-agents-pc/packages/sdkwork-agents-pc-assets/package.json',
)
const sdkworkRequire = createRequire(ASSETS_COMPONENT_PACKAGE)
const SDKWORK_CONTEXT_ALIASES = {
  i18next: sdkworkRequire.resolve('i18next'),
  'react-i18next': sdkworkRequire.resolve('react-i18next'),
}
const TAILWIND_PREFIX = '\0dsh-assets-tailwind:'
const BROWSER_BUILTIN_PREFIX = '\0dsh-assets-browser-builtin:'
const PLAIN_CSS_PREFIX = '\0dsh-assets-css:'
const VIRTUAL_SUFFIX = '.mjs'

const ASSETS_SOURCE_ROOTS = [
  resolvePath(SDKWORK_ROOT, 'apps/sdkwork-agents-pc/src'),
  resolvePath(SDKWORK_ROOT, 'apps/sdkwork-agents-pc/packages/sdkwork-agents-pc-assets/src'),
...
function virtualStyleModule(id: string, css: string): string {
  return [
    ...
    "  tag.dataset.plugin = '@deepseek-ai/dsh-client-ui-generations-assets';",
...
const withRealSdkwork: BuildFaceConfig = (env) => base(env).map(config => ({
  ...
  plugins: [
    ...(config.plugins ?? []),
    createSdkworkBrowserBuiltinsPlugin('dsh-assets-browser-builtins', BROWSER_BUILTIN_PREFIX, VIRTUAL_SUFFIX),
    {
      name: 'dsh-assets-tailwind-css',
      resolveId(source: string, importer: string | undefined) {
        const physical = physicalCssPath(source, importer)
        if (physical !== ASSETS_CSS) return null
        return TAILWIND_PREFIX + ASSETS_CSS + VIRTUAL_SUFFIX
      },
      async load(this: ResolverContext, id: string) {
        ...
        const sources: SourceEntry[] = [
          ...compiler.sources,
          ...ASSETS_SOURCE_ROOTS.map(root => ({ base: root, pattern: '**/*', negated: false })),
        ]
        ...
        return virtualStyleModule('@deepseek-ai/dsh-client-ui-generations-assets/assets-index.css', compiled)
      },
    },
    {
      name: 'dsh-assets-plain-css-inline',
      async resolveId(this: ResolverContext, source: string, importer: string | undefined) {
        ...
        const resolved = physical === undefined
          ...
          : { id: physical }
        if (resolved === null || resolved.id === ASSETS_CSS) return null
      ...
      async load(this: ResolverContext, id: string) {
        ...
        return virtualStyleModule(
          '@deepseek-ai/dsh-client-ui-generations-assets/' + cssPath,
