...
describe('CourseApp', () => {
  ...
  it('fails loud when no host adapter was configured', () => {
    expect(() => render(<CourseApp />)).toThrow('ui-course: SDKWork host runtime is not configured')
  })

  it('fails loud when no base URL is configured', () => {
    const adapter = configureCourseHost(fakeServices({ baseUrl: '  ' }))
    try {
      expect(() => render(<CourseApp />)).toThrow('ui-course: SDKWork base URL is not configured')
    } finally {
      adapter.dispose()
    }
  })
