---
description: "Desktop update discovery UI for the Electron shell: an update banner over shell.overlay and an update preferences row over settings.general.item, shipping only in the dsh-desktop-app bundle."
...
主进程持有发现、下载、安装以及持久化的 `desktop` 设置命名空间；本包只把 bridge 推送的状态镜像进两个 slot store，并经 preload 把操作路由回去，因此行为修复通常落在主进程侧。preload 缺少 `desktopBridge.updates` 时本包什么都不渲染，`follow` 通道语义也在主进程侧。
