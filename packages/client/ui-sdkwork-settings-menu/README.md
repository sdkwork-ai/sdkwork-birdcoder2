...
The settings menu over the mode rail's settings gear plus the settings modal shell it owns. The plugin occupies the `mode.rail.settings` seat that upstream's `ui-settings-general` used to hold; the web bundle patch keeps that upstream row's `id` — a profile-backed settings namespace *is* the live loader row id — and swaps only the implementation package behind it to this one, so the upstream shell never loads (its source is never touched, so upstream updates cannot conflict with this surface) while every namespace the fork re-declares stays addressable; this package re-declares every settings seat — `settings.trigger/header/action/close/section/onboarding/general.item` — so feature-owned sections, rows, and onboarding steps mount unchanged.
...
The settings modal is the package's own shell: the section nav over the `settings.section` ledger, the General section over `settings.general.item`, the loopback open-document action, and the onboarding coordinator over `settings.onboarding`. The welcome-notice acknowledgement is a volatile field of the host half's own Config, and the loader entry's id is the settings namespace — so because this package occupies upstream's `ui-settings-general` row, that field lands in the very namespace the welcome step's reader resolves, and persisted acknowledgements survive the swap.
