...
import { apply, inject } from '@deepseek-ai/dsh-client-ui-sdkwork-settings-menu/client'
import { AccountRuntime } from '../src/client/account.ts'
...
async function bench(isLoopback = true) {
  ...
  const theme = {
    getTheme: () => ({ preference: 'light', revision: 0 }),
    setTheme: vi.fn(),
  }
...
/**
 * Declare the declaration chain the way ui-layout + ui-sdkwork-app-modes do: the
 * root entry declares the mode rail and the rail entry declares the settings
 * seat; the settings child slots come from the shell occupant's own register.
 */
function declare(slots: SlotRegistry): () => void {
  const rootDispose = slots.register(
    { name: 'root', children: { 'mode.rail': { kind: 'single', scope: 'root' } } } as never,
    () => null,
  )
  const railDispose = slots.register(
    {
      name: 'mode.rail',
      children: { 'mode.rail.settings': { kind: 'single', scope: 'root' } },
    } as never,
    () => null,
  )
...
describe('ui-sdkwork-settings-menu apply', () => {
  it('declares the services it uses', () => {
    expect(inject).toEqual(['slots', 'locale', 'theme', 'connection'])
  })

  it('provides the anonymous account service', async () => {
    const b = await bench()
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const account = b.ctx.get('account')
    expect(account).toBeInstanceOf(AccountRuntime)
    expect(account!.getSnapshot()).toEqual({ signedIn: false })
  ...
  it('fills all five seats for declarations before or after apply', async () => {
    ...
    for (const [name] of SEATS) {
      expect(before.slots.entries(name)[0]!.locale).toBe('settings.menu')
    }
  ...
  it('registers the zh/en settings dictionaries and frees the seats on teardown', async () => {
    ...
    await fiber.await()
    expect(b.locale.bind('settings.menu')('title')).toBe('设置')
    expect(b.locale.bind('settings.menu')('menu.logout')).toBe('退出登录')
    b.locale.setLocale('en')
    expect(b.locale.bind('settings.menu')('close')).toBe('Close')
    b.locale.setLocale('zh')
    await fiber.dispose()
    // The (ns, locale) seats are free again — the dictionary disposer ran.
    expect(() => b.locale.register('settings.menu', 'zh', {})).not.toThrow()
    expect(() => b.locale.register('settings.menu', 'en', {})).not.toThrow()
  })
  ...
  it('mirrors theme/change into the shell hooks compartment', async () => {
    const b = await bench()
    declare(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const entry = b.slots.entries('mode.rail.settings')[0]!
    const injected = (entry.inject as unknown as () => { hooks: { theme: { getSnapshot(): unknown } }; setTheme: (id: string) => void })()
    ...
    expect(injected.hooks.theme.getSnapshot()).toEqual({ preference: 'dark', revision: 1 })
  ...
  it('removes every seat and the item declaration on teardown', async () => {
    ...
    for (const [name] of SEATS) expect(b.slots.entries(name)).toEqual([])
    expect(b.slots.spec('settings.general.item')).toBeUndefined()
    expect(b.slots.entries('mode.rail.settings')).toEqual([])
    expect(b.ctx.get('account')).toBeUndefined()
  })
})
