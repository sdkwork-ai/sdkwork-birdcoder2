...
- **Line layout** — a multiple `w:spacing/@line` counts natural lines of the typeface, the way Word lays a line out, so the body probes the font's natural line height once and turns multiples into absolute line heights; an exact rule states its pixels as they stand, and an at-least rule is a floor the typeface's own line may rise above. Word adds the space after one paragraph to the space before the next, so flow margins are precomputed per block list, the first block of a page drops its spacing, and an empty paragraph is exactly its paragraph mark's line tall.
...
- **A leader fills the run-up to a right or center stop only.** Custom stops move the following text to a left, center, or right layout the way Word's stops behave, and a stop that declares a dot, hyphen, or underscore leader fills the free width before its segment; a left stop's leader is not drawn, and a center stop balances between its neighbours rather than at its exact declared position.
