 ...
 * data and makes every container variation expressible as a test input.
 */
...
export interface ZipEntryInput {
  ...
  /** UTF-8 text the entry holds; ignored when `bytes` is present. */
  readonly text?: string
  /** Raw bytes the entry holds, for the media parts a document references. */
...
/** The namespace declarations every WordprocessingML fixture part needs. */
...
/**
 * Declare a relationships part.
 * @param rows - the `Relationship` elements.
 * @returns the complete part text.
 */
export function rels(rows: string): string {
