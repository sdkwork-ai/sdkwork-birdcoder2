...
afterEach(() => {
  ...
  delete (globalThis as Win).__DSH_CONNECTION_RECOVERY__
...
describe('connection client apply', () => {
  ...
  it('selects the desktop IPC api client and rpc when the preload bridge is present', async () => {
    ;(globalThis as Win).location = { hostname: '192.0.2.20', search: '' }
    ;(globalThis as Win).desktopBridge = {} as DesktopBridge
    const handle = await mount()
    expect(handle.api).toBeInstanceOf(IpcApiClient)
    expect(handle.isLoopback).toBe(true)
  })

  it('leaves api unset for the served web transport', async () => {
    ;(globalThis as Win).location = { hostname: 'localhost', search: '' }
    expect((await mount()).api).toBeUndefined()
  })
