...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'ui-settings-menu: dictionaries')
  ...
  // The account service seam: the anonymous default profile; the ui-iam
  // plugin replaces the source behind the same snapshot face.
  const account = new AccountRuntime()
  ctx.provide('account', account)

  // The feedback channel seam: the unavailable default; the ui-feedback
  // plugin replaces the source behind the same snapshot face.
  ...
  }), 'ui-settings-menu: metadata invalidations')
