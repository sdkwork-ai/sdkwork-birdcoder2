...
- **Feedback row is provider-gated** — the row renders only while the feedback seam's source reports `available` (the ui-feedback plugin over its configured base URL); without it the row stays hidden.
