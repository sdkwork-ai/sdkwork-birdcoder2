...
class KnowledgebaseHostRuntimeImpl implements KnowledgebaseHostRuntime {
  ...
  private readBaseUrl(): string {
    ...
    if (baseUrl === '') throw new Error('ui-sdkwork-knowledge: SDKWork base URL is not configured')
    return baseUrl
  }
...
export function KnowledgebaseApp(): ReactNode {
  ...
  if (adapter === undefined) {
    throw new Error('ui-sdkwork-knowledge: SDKWork host runtime is not configured')
  }
