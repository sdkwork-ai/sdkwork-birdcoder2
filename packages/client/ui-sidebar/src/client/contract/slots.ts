 ...
 * owns column geometry, the brand row, and global panel rows; the New Session
 * area is the `sidebar.actions` registrants' (the shell keeps its built-in
 * capsule as the empty-list fallback); everything between it and the list
 * bottom is the `sidebar.workspaces` registrant's (ui-workspace), and the foot
 * is the `sidebar.settings` registrant's (ui-settings), followed by optional
