...
export function apply(ctx: ClientContext): void {
  ...
  ctx.slots.inject('sidebar', () => ctx.slots.register({
    name: 'sidebar',
    locale: NS,
    // The shell owns geometry and global panel rows; the quick entries (fork
    // plugins) fill the New Session button area, ui-workspace registers the
    // whole browsing region (header, search, session list, workspace dialogs),
    // ui-settings registers the foot trigger + settings panel.
    children: {
      'sidebar.brand.mark': { kind: 'single', scope: 'root' },
      'sidebar.brand.name': { kind: 'single', scope: 'root' },
      'sidebar.actions': { kind: 'list', scope: 'root' },
