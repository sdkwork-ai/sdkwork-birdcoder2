...
const bundle = clientBundle('@deepseek-ai/dsh-client-ui-sdkwork-pdf-preview', ['lib/types/index.js'])
const require = createRequire(import.meta.url)
const workerSpecifier = 'pdfjs-dist/build/pdf.worker.min.mjs?raw'
const workerModule = '\0sdkwork-pdf-worker.mjs'
...
const pdfWorker: NonNullable<UserConfig['plugins']> = [{
  name: 'sdkwork-pdf-worker-source',
  resolveId(source) {
...
export default (options: Parameters<typeof bundle>[0]): UserConfig[] => bundle(options).map(config =>
  config.name?.endsWith('/client') === true ? {
    ...
    define: { ...config.define, __SDKWORK_PDFJS_ASSETS__: pdfAssets() },
  } : config,
)
