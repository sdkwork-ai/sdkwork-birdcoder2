/** ui-sdkwork-assets apply wiring: the rail entry and the placeholder page, each
 * keyed by the `assets` mode id, registered once their slot declarations
 * are on the ledger; teardown cascades. */
import { Context } from '@deepseek-ai/cordis'
import { describe, expect, it } from 'vitest'
import { SlotRegistry } from '@deepseek-ai/dsh-client-runtime/client'
import { LocaleRuntime } from '@deepseek-ai/dsh-client-locale/client'
import { apply, inject } from '@deepseek-ai/dsh-client-ui-sdkwork-assets/client'
import type {
  AssetsPageInjected, AssetsRailEntryInjected,
} from '@deepseek-ai/dsh-client-ui-sdkwork-assets/client'
import { AssetsRailEntry } from '../src/client/RailEntry.tsx'
...
describe('ui-sdkwork-assets apply', () => {
  it('declares the services it uses', () => {
    expect(inject).toEqual(['slots', 'locale'])
  })

  it('registers the rail entry and the page keyed by the assets mode id', async () => {
    const b = await bench()
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const [entry] = b.slots.entries(RAIL_ENTRY)
    expect(entry?.options.key).toBe('assets')
    expect(entry?.component).toBe(AssetsRailEntry)
    expect(entry?.locale).toBe('assets')
    expect((entry!.inject as unknown as () => AssetsRailEntryInjected)().mode).toBe('assets')
