 ...
 *
 * The return side carries `Promise<ReactNode>` explicitly: @types/react 19
 * widens `FC`'s return to `ReactNode | Promise<ReactNode>` (async client
 * components), while react's own `ReactNode` only admits
 * `Promise<AwaitedReactNode>` — without the extra branch an `FC` no longer
