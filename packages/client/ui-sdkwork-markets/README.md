---
description: "Markets app-mode plugin: the market quick entry and the keyed center-column page whose header tabs host the Plugins, Experts, Skills, and Connectors markets plus the Local plugins and Installed inventory views over the running application's own plugin tree, and whose Plugins tab carries the add flows (skill-driven plugin creation, add-market entry dialog)."
...
Two tabs are not storefronts: **Local plugins** and **Installed** are views over this deployment's own plugin tree, read through the Host inventory Remote — the same read-only source the Settings plugin-inventory tab uses. Local plugins lists every Loader entry with its origin (local path or published package), effective enablement, and root-fiber phase; Installed lists the entries in force and offers a Settings affordance per row. The market therefore stays a single roster: what this application runs, not a second list that can silently diverge from it.
