---
description: "Markets 应用模式插件：插件市场快捷入口与 keyed 的中心列页面，页头 tab 承载插件、专家、技能、连接器市场，插件 tab 提供添加入口（skill 引导的插件创建与插件市场录入弹窗）。"
...
插件通过 `ctx.slots` 注册侧边栏入口与 keyed 页面、通过 `ctx.locale` 注册字典、通过 `ctx.layout` 完成模式切换。页面是公共的：未登录即可渲染，不挂载 IAM 会话面，因此登录前即可浏览目录。添加入口需要 sessions 与 workspaces 服务：派发时把框架切到 `code` 模式、运行共享的新会话流程、（有界）等待新会话成为当前会话，再把合成提示词作为排队文本轮发送进去。
