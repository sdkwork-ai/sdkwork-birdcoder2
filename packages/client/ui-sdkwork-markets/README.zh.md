---
description: "Markets 应用模式插件：插件市场快捷入口与 keyed 的中心列页面，页头 tab 承载插件、专家、技能、连接器市场；插件 tab 本身是子根，可在云端目录与基于当前应用插件树的官方/已安装视图之间切换；插件 tab 还提供添加入口（skill 引导的插件创建与插件市场录入弹窗）。"
...
插件 tab 本身是一个子根：主 tab 栏下方的一行 chip 把它分为**云端目录**（默认落地视图）与覆盖本部署自身插件树的两个视图——通过 Host inventory Remote 读取，与设置中心插件清单 tab 相同的只读数据源。细分隔线把云端 chip 与「本应用」一对 chip 分开，让整行读作「商店 | 本应用插件」。**官方插件**按上游插件管理页面的两组划分，判定式与卡片粒度都一致：**官方**组是 `optional && !installed`（安装自带、供你选择启用），**已安装**组是 `installed || !optional`；内置的 profile 插件包两组都不进，空组不占位置。每张卡片自带该插件包的开关、卸载入口，以及默认折叠的它所声明的插件行——因此某个条目的行只会出现在声明它的那张卡片里。**已安装**视图把当前生效的条目平铺成名单，并为每行提供设置入口。因此市场始终是同一份名单：本应用正在运行的插件，而不是一份会与它静默分叉的清单。其余主分类（专家、技能、连接器）保持四项 tab 体验——子 chip 行只属于插件页。
