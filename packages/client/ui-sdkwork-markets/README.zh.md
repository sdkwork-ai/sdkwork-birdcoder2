---
description: "Markets 应用模式插件：插件市场快捷入口与 keyed 的中心列页面，页头 tab 承载插件、专家、技能、连接器市场；插件 tab 本身是子根，可在云端目录与基于当前应用插件树的本地/已安装视图之间切换；插件 tab 还提供添加入口（skill 引导的插件创建与插件市场录入弹窗）。"
