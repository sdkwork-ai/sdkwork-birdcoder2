...
分层与协议决策记录在 [GUI 分层与 RPC 协议 RFC](../../../.agents/notes/archived/architecture/2026-07-19-gui-layering-and-rpc-protocol.md) 中；浏览器侧消费架构记录在 [Web 客户端架构 RFC](../../../.agents/notes/archived/architecture/2026-07-19-gui-web-client-architecture.md) 中。
...
`session.history` 会读取已附加 Session 的内存状态，或通过持久化检查冷日志，而不会恢复或发布 agent，然后按追加来源的消息边界分页：`maxMessages` 统计以追加方式进入 surface 的 `user/message` 和 `assistant/message` 事件，因此仅供模型使用的替换副本不占用配额。每一页仍是一段连续的原始事件区间，从而让压缩（compaction）的仅日志 `compaction/summary` 记录与引用它的替换留在同一页。一页还设有原始事件数上限（12,000）：单个消息组可能携带数万个流式 chunk，如此大的页会在打开 transcript 时拖垮两个进程（序列化、IPC 传输、JSON.parse、fold），因此切口落在越过消息配额或事件预算的第一条消息组上——始终保留完整消息组，绝不在消息中间切断。
...
`agentPreset.list` 领域向浏览器暴露部署的 preset 名单，使其在开启会话时能够提供选择；每一行携带它是否为当前默认值、它公布过的显示名与描述（若有），以及——当该 preset 无法组装会话时——一条 `broken` 原因：加载失败的 preset 仍占着它的 id，界面必须能展示它，而不是把它端出来然后在会话启动时失败。未组装任何 preset 的部署返回空名单而非错误，因为共用宿主组装本身就是一种有效部署。`agentPreset.select` 用另一个 preset 重组某个会话的 agent，且仅在会话空白时允许：一旦跑过任何轮次，那段历史就是在该 preset 的工具下产生的，替换会留下无法执行的已记录的工具调用，此时返回 `agent-preset-locked`。agent 与会话都不销毁——只替换组装，且替换失败会恢复原来的组装。

preset 是声明的，不在这条协议上创作。上游用「在活动 profile 中声明为普通行」的 preset 取代了目录式 preset 创作模型（“preset 没有各自独立的路径”），因此 `read`／`copy`／`openDocument`／`remove` 与名单里的 `trust` 划分都已消失：既没有可供读取、复制或交给打开器的按 preset 划分的目录，写组装也变成一项 profile 补丁编辑，而那已由 `settings.*` 领域承载。`list` 与 `select` 保持为普通方法而非环回固定方法——名单只携带 id 与元数据，每个 preset 选择器都需要它；而选择一个 preset 并不比 `session.create` 自带的 `agentPreset` 多给任何能力，何况默认 preset 本就带着 bash。本领域已无任何被固定的方法，为一条没有路由的方法补回固定项，只会是一道幻影护栏而非篱笆。
