...
describe('JsonlSessionPersistence: immutable format generations', () => {
  ...
  it('serializes concurrent historical migrations and shares one in-flight pass per id', async () => {
    ...
    for (const header of [first, second]) {
      const sourcePath = historicalLogPath(root, header.cwd, header.id)
      await mkdir(dirname(sourcePath), { recursive: true })
      // Historical v1 rows are stored verbatim: the current encoder would
      // refuse their sourceEventSeqs, which only the migration decodes. The
      // step-first row order is the one a v2 system head can be prepended to.
    ...
    for (const restored of [firstRead, firstJoin]) {
      ...
      expect(restored.events).toEqual(migratedOneTurnLog())
    }
    ...
    for (const header of [first, second]) {
      ...
      // A read migration serves the decoded log without publishing a
      // successor generation; only a write open materializes one.
  ...
  it('does not join an in-flight historical preparation for an older source revision', async () => {
    ...
    // The appended source changes its revision, so the second open must not
    // join the first pass; the backend serializes the two physical reads, so
    // the second starts only once the paused first read finishes.
    pause.release()

    const [first, second] = await Promise.all([firstOpening, secondOpening])
    try {
      // Each pass may issue several physical reads (header probe plus body
      // decode), and the backend serializes passes on one id.
      await vi.waitFor(() => { expect(readTally.bySuffix.get(sourcePath) ?? 0).toBeGreaterThanOrEqual(2) })
