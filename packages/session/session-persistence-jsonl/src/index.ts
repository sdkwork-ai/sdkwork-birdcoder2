...
class JsonlSessionPersistence extends SessionPersistence {
  ...
  private async prepareStoredMigration(
    ...
    // Historical-generation migrations are rare one-time passes, but every
    // pass materializes the whole log (decompress, parse, per-step ownership
    // snapshots, re-encode), so its transient memory is a large multiple of
    // the session size. Concurrent opens of the same session share the
    // preparation above; opens of different sessions still run one at a time
    // per storage, because desktop hydration lists every session and
    // unbounded parallel migrations multiply one pass's peak memory until
    // they can exhaust the process allocator.
    let prepared: Awaited<ReturnType<typeof prepareJsonlMigration>>
    let validateRelatedSources: () => Promise<void>
    try {
      // Upstream's preamble: the sibling artifacts this generation's catalog
      // folds in must still be the same set when the migration settles, so its
      // membership is captured here and re-checked through the validator.
      ...
      // FORK DIVERGENCE: the pass is serialized on this storage's migration
      // queue, so concurrent sessions cannot multiply one pass's peak memory.
      const decode = this.migrationQueue.then(() => prepareJsonlMigration({
        sourcePath: selected.sourcePath,
      ...
      // The queue only orders work; it never swallows or forwards results.
      this.migrationQueue = decode.then(() => undefined, () => undefined)
      prepared = await decode
    } catch (error: unknown) {
      throw this.generationFailure(id, selected, error)
    }
