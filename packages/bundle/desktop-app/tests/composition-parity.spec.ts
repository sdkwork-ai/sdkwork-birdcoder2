...
const ADDED_ROWS = ['desktop-carrier', 'desktop-connection', 'desktop-app', 'window-controls', 'update-banner'] as const
...
describe('desktop and Web plugin composition parity', () => {
  it('declares the update UI package mounted by the desktop bundle', () => {
    ...
    expect(manifest.dependencies?.['@deepseek-ai/dsh-client-ui-updater']).toBe('workspace:^')
  })
  ...
  it('limits changed and added rows to the documented desktop transport and shell plugins', () => {
    ...
    expect(ADDED_ROWS.map(id => desktop.get(id))).toEqual([
      ...
      { id: 'desktop-app', name: '@deepseek-ai/dsh-desktop-app' },
      { id: 'window-controls', name: '@deepseek-ai/dsh-client-ui-window-controls' },
      { id: 'update-banner', name: '@deepseek-ai/dsh-client-ui-updater' },
    ])
  })
