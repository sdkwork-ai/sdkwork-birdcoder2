...
- **Profile module fallback.** Bare plugin specifiers resolve through the Loader from the config directory. Plain Node maintains one symlink per package in the installation dependency closure. A packaged executable instead reads each installed export map with Node ESM conditions and writes real proxy packages that re-export virtual module URLs, because an operating-system symlink cannot enter pkg's `/snapshot` tree. Missing exports stay unavailable, malformed maps fail startup, and a cross-process writer lock replaces stale entries without exposing partial proxies. A selected external bundle absent from the installation closure receives a profile-local `.dsh-module-fallback` link; existing pnpm entries win, projected links are excluded from later closure discovery, and cleanup removes only dsh-owned links. When Node loader internals are unavailable, `bareModuleBaseUrl` resolves the package with ESM `import` conditions before Include imports its URL; a package with distinct `import` and `require` exports therefore selects the same entry on Web and embedded launchers.
- **Launcher-owned home and runtime overlays.** `loadLayeredEnv` builds the product CLI's frozen inherited > project `.env` > user `.env` snapshot, optionally from a launcher-resolved Harness home, and `boot` exposes the default or launcher-owned `dshHomePath(...segments)` to Loader `!!js` config expressions. `loadBundleLayer` uses the same resolution for a launcher-owned runtime overlay without persisting that bundle in the profile manifest. `watchUserPatches` watches the named patch file through Cordis HMR when available, otherwise through an exact-path file watcher; a rejected read, parse, or Loader candidate leaves the last good tree running and broadcasts `hmr/config-update-failed(filename, Error)` after logging it.
