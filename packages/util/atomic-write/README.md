...
await writeFileAtomic('<home>/.dsh/cordis.patch.yml', text, { mode: 0o600 })
```
...
await withFileLock('<home>/.dsh/cordis.patch.yml', async () => {
  const previous = await readCurrent()
  await writeFileAtomic('<home>/.dsh/cordis.patch.yml', render(previous), { mode: 0o600 })
})
```
