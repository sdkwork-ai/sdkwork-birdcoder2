...
export async function revealNativePath(
  ...
  if (manager === 'explorer') {
    ...
    // FORK DIVERGENCE: Explorer's switch parser reads `/select,<target>` as ONE
    // command-line token; when the switch and the target arrive as separate argv
    // entries Explorer ignores the reveal and opens its default folder instead
    // (upstream's own `runExplorer` doc names the joined form). Keep them joined.
