...
`staging` 与 `production` 的 token 必须来自私有密钥源。`development` 只会为 loopback 网关自动生成一次性本地 JWT；`test` 还需要 `--allow-test-token-generation` 且目标网关同样必须是 loopback。远端 development/test 网关应使用 `.sdkwork.local.env` 等已 provision 的真实 token。`ui-sdkwork-env` 插件将所选环境的网关 URL 与 access token 投影进每个 SDKWork 集成插件的配置。
