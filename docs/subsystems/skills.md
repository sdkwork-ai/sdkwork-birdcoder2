...
`dsh-skill-badge` registers one immutable `bundled` candidate at `BUNDLED_SKILL_RANK` and exposes its packaged asset directory through `resourceBase`. The shipped CLI declares the plugin disabled, so enabling its composition row is an explicit opt-in. The SDKWork fork's `dsh-sdkwork-builtin-skills` is the same shape for a packaged directory root instead of a virtual candidate: it mounts the shared filesystem provider over its own `assets/skills` directory at that rank, and the `dsh-web-app` bundle patch inserts its row, so every preset reads the fork's `birdcoder-*` scene skills in both the browser and desktop profiles.
