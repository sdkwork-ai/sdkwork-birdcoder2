...
| `tsconfig.client.tests.json` (also for a Client package) | mirror the same `references` entry — the client test aggregate carries the same project list as the build solution because its checks redirect imports into referenced projects to their emitted declarations; `scripts/client-tsconfig.spec.ts` enforces the two lists stay equal |
