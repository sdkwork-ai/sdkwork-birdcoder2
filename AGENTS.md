...
grep -c 'class="dsh-mark"' website/.vitepress/config.ts   # 1: the docs nav lockup renders the bird mark
grep -rn "DeepSeek Harness" website/.vitepress/config.ts   # nothing: the site identity is the fork's name
ls website/public/wordmark.svg 2>/dev/null   # nothing: the upstream name is gone as glyph paths, not just restyled
