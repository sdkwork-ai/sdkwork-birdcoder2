...
`@deepseek-ai/dsh-client-ui-sdkwork-drive` 是[应用模式栏](2026-08-16-sidebar-app-modes.md)中的独立模式插件。其条目调用 layout store 既有的 `setMode('drive')` 动作，其 keyed 的 `mode.page` 注册在中心列挂载 SDKWork 应用。该模式是瞬态布局状态：Drive 导航既不增加浏览器路由，也不持久化 BirdCoder 偏好。
