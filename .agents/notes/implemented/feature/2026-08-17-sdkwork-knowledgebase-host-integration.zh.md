...
`@deepseek-ai/dsh-client-ui-sdkwork-knowledge` 在[应用模式栏](2026-08-16-sidebar-app-modes.zh.md)中保持为独立模式插件。其条目调用 layout store 既有的 `setMode('knowledge')` 动作，keyed 的 `mode.page` 注册则在中心列挂载 SDKWork 应用。模式属于瞬时 layout 状态：知识库导航既不添加浏览器路由，也不增加持久化 BirdCoder 偏好。
