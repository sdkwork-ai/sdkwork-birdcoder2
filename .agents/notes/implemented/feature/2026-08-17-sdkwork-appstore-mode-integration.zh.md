...
`@deepseek-ai/dsh-client-ui-sdkwork-appstore` 是[应用模式栏](2026-08-16-sidebar-app-modes.zh.md)中的独立模式插件。它持有应用商店字形与文案，注册 keyed 的 `appstore` 模式栏条目和页面，并使用 layout store 现有的模式操作。选择应用商店仍属于瞬时 layout 状态，不添加浏览器路由或持久化偏好。
