...
| Keep App Store as a `ui-app-modes` placeholder | The shell package would own SDKWork business behavior and credentials instead of a feature-owned entry and page |
| Add App Store-specific environment or auth settings | `ui-env` and `ui-iam` already own deployment and identity; copied state would diverge during environment switches and sign-out |
