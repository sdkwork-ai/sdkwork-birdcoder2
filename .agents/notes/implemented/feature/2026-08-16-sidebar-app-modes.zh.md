...
GUI 呈现微信桌面风格的左侧模式栏，其活动条目跟随框架的模式状态；工作、视频、图片与资产使用功能自有的占位页，[应用商店](2026-08-17-sdkwork-appstore-mode-integration.md)、知识库与 Token Plan 则挂载各自功能的应用。切回代码会恢复会话表面（其状态留在运行时对象层）。通用设置里的侧边栏可见性行会持久化该偏好，并在启动与变更时应用。代价：每个框架都为模式栏预留 56px（让步链会将其净扣除）；模式切换会卸载会话表面（重挂载即恢复）；持久化偏好只在 scope 首次就绪时应用一次，因此之后的设置文档变更不会在开关被拨动前重新收起已展开的侧边栏。
