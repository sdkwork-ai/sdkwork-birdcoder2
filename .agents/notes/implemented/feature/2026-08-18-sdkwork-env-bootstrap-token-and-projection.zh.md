...
- **启动 token ensure**（`@deepseek-ai/dsh-sdkwork-env-bootstrap`，新 `packages/boot` 包）：`dsh` CLI 与桌面壳在分层 `.env` 加载后调用 `ensureSdkworkBootstrapToken`。优先级：显式配置的 `SDKWORK_ACCESS_TOKEN` → 注册产物 `.sdkwork.local.env` → 生成——development 自动生成一次性本地 JWT 并写入被 gitignore 的 `.env.standalone.development.bootstrap.local` 覆盖文件，test 需要 `--allow-test-token-generation`，staging/production 对私有 secret 来源 fail closed。确保到的 token 会被物化进 `process.env` 供 ui-env host 投影使用。JWT 创建、manifest 查找与 env 文件解析全部复用 `@sdkwork/iam-credential-entry`（动态加载，因此没有 SDKWork 兄弟仓库的 harness 仍可启动）。未打包的 `pnpm desktop:dev` 会先调用 `applySdkworkDesktopLaunchEnv`，因此即使 Electron 的 cwd 是 `apps/desktop` 也会投影 development 网关；打包构建套用生产网关（[desktop:dev 与打包网关](../bug-fix/2026-08-18-desktop-dev-development-gateway.zh.md)）。
...
- 本地测试的环境切换现在是 `cp .env.standalone.test .env` 加重启：CLI/desktop 的 ensure 步骤生成 token，ui-env 把环境、base URL 与 token 投影进每个 SDKWork 插件。
...
包级单元测试覆盖 ensure 阶梯（unconfigured / configured / registered / generated 幂等 / test 授权 / production fail-closed / manifest 缺失）于 `packages/boot/sdkwork-env-bootstrap/tests`，投影（档位映射、URL 优先级、空字段省略、user 层权威）于 `packages/client/ui-env/tests`。CLI 已端到端实测：development 生成带 manifest claims 的覆盖 JWT，test 无开关时给出授权提示，`admin:bootstrap:app` 无超管凭据时 fail loud。文档门禁（翻译配对、预算、链接、note 格式）校验双语文档。
