...
- 本地测试的环境切换现在是 `cp .env.standalone.test .env` 加重启：`pnpm env:token:ensure`（以及 CLI/desktop 的 ensure 步骤）只会为 loopback development/test 网关生成 fixture token；否则 bootstrap 凭据改由注册结果或其他私有 token 来源提供。ui-env 仍会把环境、base URL 与最终 token 投影进每个 SDKWork 插件。
