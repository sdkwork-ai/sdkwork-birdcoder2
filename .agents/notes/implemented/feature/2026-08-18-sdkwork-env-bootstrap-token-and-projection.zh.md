...
包级单元测试覆盖 ensure 阶梯（unconfigured / configured / registered / generated 幂等 / test 授权 / production fail-closed / manifest 缺失）与桌面启动 env（子目录上溯、development 文件、packaged production、继承 URL）于 `packages/boot/sdkwork-env-bootstrap/tests`，投影（档位映射、URL 优先级、空字段省略、user 层权威）于 `packages/client/ui-env/tests`。CLI 已端到端实测：development 生成带 manifest claims 的覆盖 JWT，test 无开关时给出授权提示，`admin:bootstrap:app` 无超管凭据时 fail loud。文档门禁（翻译配对、预算、链接、note 格式）校验双语文档。
