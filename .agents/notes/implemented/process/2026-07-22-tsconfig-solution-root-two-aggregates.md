...
The client test aggregate later moved out of the build solution: `tsconfig.client.json` is now a program-less reference driver for the Client tsc/tsdown pass, and the aggregate lives in `tsconfig.client.tests.json` (same references, run by the typecheck gates only) — see the [client test aggregate split note](2026-09-07-client-test-aggregate-out-of-build-solution.md).
