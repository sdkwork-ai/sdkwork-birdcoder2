...
10. **Re-converge the release family version (2026-09-11 lesson).** `DshFamily.verifyVersions` requires one shared version across every publishable member (`packages/!(experimental)/*/package.json` plus `apps/*/package.json`, minus `private: true`), and both merge parents enforce it. Upstream's bump lands only on the files upstream edits: a fork-only member has no upstream counterpart, so it never conflicts and keeps the previous version line. The 2026-09-11 sync left 49 fork manifests at `0.1.5-alpha.1`/`0.1.5-alpha.2` while upstream moved 266 to `0.1.5-rc.2`, the drift was committed, and every `release:pack --family dsh` aborted on the family version gate before packing anything. After resolving the merge, read the merged root version, write it onto every lagging member manifest, then `pnpm install --lockfile-only` and confirm the family reports one version.
11. **Advance the sibling pins with the lockfile (2026-09-11 lesson).** CI clones the `scripts/sdkwork-sources.manifest.json` pins into `../sdkwork-*` and then runs `pnpm install --frozen-lockfile`, but `pnpm-lock.yaml` records the specifiers resolved from the *local* sibling checkouts. A pin behind its local HEAD therefore installs cleanly on the maintainer's machine and fails on every CI runner with `ERR_PNPM_OUTDATED_LOCKFILE`, which reads as a lockfile problem rather than a pin problem. Push the sibling repositories, point each pin at the pushed HEAD, and only then re-record and verify the lockfile — before pushing the merge.
...
| `package.json` version line | theirs, then propagate to fork-only members | fork does not publish; the upstream line keeps the next sync small, and the family version gate covers the members upstream never edits (step 10) |
