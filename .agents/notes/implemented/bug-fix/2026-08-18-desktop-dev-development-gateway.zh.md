...
测试省略 `sdkworkEnv`，因此隔离的 `cwd` 按原样使用。ui-env 仍通过 settings `base` 层投影；`$DSH_HOME/settings.yaml` 中用户编辑过的 `ui-env:` 分区仍然权威（[env 引导与投影](../feature/2026-08-18-sdkwork-env-bootstrap-token-and-projection.md)）。
