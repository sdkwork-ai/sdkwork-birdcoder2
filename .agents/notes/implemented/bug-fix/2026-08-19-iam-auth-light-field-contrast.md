...
**Ship the border and rail override as a plain `.css` import, or stretch the QR wrapper with Tailwind `[&>div]:h-full` on `slotProps`.** Rejected: tsdown injects CSS Modules only, and the web Tailwind `@source` list does not include `ui-sdkwork-iam`, so neither path reaches the running overlay.
