...
// notice); an absent `ui-sdkwork-env` section resolves the base URL to the default
// api.birdcoder.com origin, which drives the account rail entry and the modal
// auth surface; a configured `ui-sdkwork-env` profile base URL swaps in the stub
// server.
...
describe('web e2e: the SDKWork IAM integration', () => {
  ...
  beforeAll(async () => {
    ...
    // Boot with an EXPLICITLY empty ui-sdkwork-env production profile: the schema
    ...
    await writeFile(join(scaffold.harnessHome, 'settings.yaml'), "ui-sdkwork-env:\n  production:\n    apiBaseUrl: ''\n")
  ...
  it('opens the sign-in dialog from the settings menu even before the IAM base URL is configured', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-ui-sdkwork-iam-menu-unconfigured'))
  ...
  it('treats an absent ui-sdkwork-env section as the default api.birdcoder.com base URL', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-ui-sdkwork-iam-default-base-url'))
  ...
  it('serves the configured ui-sdkwork-env profile base URL to the auth surface', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-ui-sdkwork-iam-stub-base-url'))
...
`)
  ...
  it('mounts the full-page auth surface in the account mode', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-ui-sdkwork-iam-account-page'))
...
`)
  ...
  it('opens the modal sign-in host from the settings-menu account seam', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-ui-sdkwork-iam-modal'))
    await dismissOnboarding(page)
    const trigger = page.getByRole('button', { name: '设置', exact: true })
  ...
  it('adapts the auth surface to the harness dark theme', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-ui-sdkwork-iam-dark-mode'))
