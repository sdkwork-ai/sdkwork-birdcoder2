...
export function createElectronBuilderConfig(
  ...
  return {
    ...
    files: [
      // FORK DIVERGENCE: upstream lists every preload entry by name; the fork
      // ships `lib/*.js` and `lib/*.cjs` instead so a new preload never needs a
      // second edit here, and adds only the welcome subdirectory a top-level
      // glob cannot reach.
      'lib/*.js',
      'lib/*.cjs',
      'lib/welcome/**/*',
    ...
    extraResources: [
      ...
      // FORK DIVERGENCE: `src/main.ts` reads the About-panel icon from
      // `process.resourcesPath/icon.png` in a packaged app. Upstream dropped that
      // copy when it renamed the artwork, so the About panel fell back to no icon
      // at all; this ships the BirdCoder raster generated by scripts/generate-icons.mjs.
