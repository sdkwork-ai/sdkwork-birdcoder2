...
The first application-menu command, **About BirdCoder**, opens Electron's native About panel with the application icon, product name, and installed release version. The menu follows the Desktop shell locale. macOS reads the icon from its application bundle, so an unpackaged development launch displays Electron's icon; Windows receives the packaged PNG.
...
Every icon this package ships derives from one raster — `apps/web/public/favicon.png`, the canonical BirdCoder mark. `pnpm --dir apps/desktop run generate-icons` writes the application icons into `build/` (the multi-size ICO for the Windows application, installer and uninstaller, the ICNS for macOS, and the PNG raster for Linux) plus the 1024×1024 About-panel rasters `resources/icon.png`, `resources/icon-windows.png` and `resources/icon-macos.png`; `extraResources` copies the Windows variant to `resources/icon.png`, which is where `src/main.ts` reads the About-panel icon in a packaged app. `pnpm --dir apps/desktop run generate-installer-brand` writes the installer's own rasters into `installer/assets/`. The repository keeps no designer SVG original: upstream's `resources/icon*.svg` held the upstream artwork, and a copy left behind is a path for a merge or a hand export to put it back. The installer's welcome pages draw `installer/assets/brand*.png` directly, and the uninstaller's welcome and finish pages share `installer/assets/uninstaller-sidebar.png`, converted to a 164×314 BMP during preparation. The ICO container exists because electron-builder needs one icon carrying every Windows size ([Windows icon requirements](https://learn.microsoft.com/en-us/windows/apps/design/iconography/app-icon-construction)).

The macOS ICNS carries representations up to 1024 pixels and is a flattened icon, not an Icon Composer document. It reuses the same trimmed, transparent square the other platforms get, with no rounded plate baked in, so a squircle appearance would need per-platform artwork. Apple's [app icon guidance](https://developer.apple.com/design/human-interface-guidelines/app-icons) describes unmasked layers for Icon Composer, and those inputs require a separate macOS export that must not reuse the flattened ICNS artwork. Verify Finder and Dock appearance on supported macOS versions before release.
...
Windows and Linux windows carry the BirdCoder product mark: `build/icon.{png,ico,icns}` come from the canonical `apps/web/public/favicon.png` through `pnpm --dir apps/desktop run generate-icons`, the shell passes `build/icon.png` to every window it opens, and electron-builder names those files as the application, installer, and AppImage icons instead of relying on its defaults.
