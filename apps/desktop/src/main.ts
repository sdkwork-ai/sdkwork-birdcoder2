...
function createWindow(preload: string, show = false): BrowserWindow {
  // Fork brand: every shell window carries the BirdCoder raster resolved from
  // the application directory (see app-icon.ts and packages/client/ui-primitives).
...
async function main(): Promise<void> {
  ...
  // The app-window preload reports the desktop plugin manager as unavailable
  // while the shell is unpackaged (main rejects plugin transactions then); it
  // reads the same derived project path the launcher used to publish.
  if (development !== undefined) process.env.DSH_DESKTOP_DEV_PROJECT_DIR = development
  ...
  // (the native menu bar is dropped on Windows/Linux), so the product renderer
  // invokes them over IPC from the main window's preload bridge.
  ...
  if (process.platform === 'darwin') {
    // macOS requires a native application menu (window management, edit roles);
    // it renders in the system menu bar, not as a second header.
    ...
    // header above the web shell's own header; every entry it carried (desktop
    // plugins, check for updates, quit) lives in the settings popover now, so
    // drop the native menu entirely.
