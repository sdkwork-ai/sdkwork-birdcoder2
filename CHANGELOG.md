...
## 0.1.5-alpha.2（2026-09-10）

本版本同步上游 deepseek-harness 0.1.5-alpha.2（merge b2e3b2a012），一并带入 0.1.3-alpha.2 与 0.1.5-alpha.1 的全部变更（共 1141 个上游提交）。BirdCoder 品牌资产（BirdLogo、favicon）、RailTooltip 行为组件、全部 `ui-sdkwork-*` / `sdkwork-*` fork 包经逐项核验保持不受影响。

### 同步上游 主要变更

#### 新增功能

- 桌面工作区引入可逆停靠引擎（dockkit）：支持指针交互的面板停靠、分条与芯片呈现、关闭策略和房间读取。
- 侧边栏升级为多标签工作区：标签导航、全屏外壳、注入信息面板、惰性加载的工作区文件树标签、实时标签标题与文件类型图标、末标签关闭规则，支持直接预览图片文件。
- 工作区文件体系成型：双面文件 API 与 Host 解析资源、Client 资源注册表与保留订阅、有界字节区间读取（本地与 E2B 沙箱实现）、远端暴露工作区文件操作。
- 交付物卡片统一：不可变文件交付以下载卡片呈现，工件卡片提供原生文件操作，交付产物可从侧边栏资源打开，路径相对工作区显示。
- 文本预览支持分页文件标签并保留阅读器状态；文档预览框架可扩展（注：上游在 0.1.5-alpha.2 中撤回了 Mermaid/Graphviz/SVG/HTML 代码围栏预览，本版本不含该功能）。
- 子代理可观察父级拥有的子代理目录；人机协同收件箱支持历史内更新与持久化恢复（durable inbox recovery）。
- 会话格式 V3 推进：V2 提示词迁入 V3 系统面，身份（identity）完成 V2 到 V3 迁移并新增写入骨架。
- 统一反馈入口：`/feedback` 与点踩打开同一个带分类的反馈对话框。
- 桌面端支持按目标平台的打包与自动更新路径，并新增上传脚本。
- 统计条改为两个图标胶囊，点击打开统计对话框。
- 技能支持用 Playwright 视频录制浏览器 GIF；native 层新增预编译 Node-API flock 支持；SDK 默认停用 `str_replace_editor`。

#### 体验优化

- 斜杠命令描述本地化；共享文件类型图标集与链接图标统一全界面表达。
- 侧边栏面板条目简化，新增全局侧边栏面板；右列响应式布局与宽度让步。
- 附件呈现统一，文件随提交生命周期流式传输，命令流可接收文件。
- 性能：会话恢复转移冻结持久化读、agent-loop 复用已验证的消息冻结、首次读取即物化实时观察事件、按紧凑记录读取内嵌 Assistant 流、迁移校验器依赖打包化。

#### 问题修复

- 修复 webworker 文件句柄 chmod 与 bigint 句柄身份问题。
- 修复已交付文件按查看会话范围打开、工件操作在重连与键盘操作后保留等问题。
- 修复侧边栏空分栏拒绝、Office 文件图标保留、惰性侧边栏重放等问题。
- 大量稳定性修复（295 个 fix 提交）覆盖会话、agent-loop、子代理、LLM 与桌面打包路径。
