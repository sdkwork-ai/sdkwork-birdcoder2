...
COPY --from=sdkwork-ecosystem sdkwork-catalog /sdkwork-catalog
COPY --from=sdkwork-ecosystem sdkwork-documents /sdkwork-documents
...
COPY --from=sdkwork-ecosystem sdkwork-cloudrouter /sdkwork-cloudrouter
COPY --from=sdkwork-ecosystem sdkwork-models /sdkwork-models
...
RUN mkdir /runtime \
  ...
  && node --input-type=module -e "import { accessSync, constants } from 'node:fs'; import { launcherPath } from '@deepseek-ai/node-addon-system/landlock-run'; accessSync(launcherPath(), constants.X_OK)" \
