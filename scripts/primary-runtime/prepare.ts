...
/**
 * FORK DIVERGENCE (win-arm64 is a fork packaging target upstream never builds,
 * and the fork carries this retry from the pre-extraction era of
 * `apps/desktop/scripts/prepare-primary-runtime.ts`): Windows target names other
 * than `win-x64` still ship a `.zip` and a `node.exe`, and Windows real-time
 * antivirus can still hold scan handles on the freshly extracted interpreters
 * when the staging directory is reclaimed. Removing the staging directory is
 * best-effort: it is disposable OS temp space, so a directory that stays locked
 * through the retries is left behind with a warning instead of failing the
 * packaging run it staged.
 * @param staging - Disposable staging directory to remove.
 */
...
export async function preparePrimaryRuntime(options: PreparePrimaryRuntimeOptions): Promise<void> {
  ...
  try {
    ...
    if (!options.pythonOnly) {
      ...
      // FORK DIVERGENCE: every Windows target ships a `.zip` and a `node.exe`, not only `win-x64`.
    ...
    const manifest: PrimaryRuntimeManifest = {
      ...
      // FORK DIVERGENCE: `win-arm64` is a fork target, so the platform follows the target prefix.
      platform: target.startsWith('win') ? 'win32' : target.startsWith('linux-') ? 'linux' : 'darwin',
