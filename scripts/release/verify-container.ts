...
function main(): void {
  ...
  assert(dockerfile.includes('native/landlock-run/scripts/pack-release.mjs /packs/landlock'), 'Dockerfile must use the complete Landlock release packer')
  assert(!dockerfile.includes('native/landlock-run/packages/entry pack'), 'Dockerfile must not pack only the Landlock entry package')
