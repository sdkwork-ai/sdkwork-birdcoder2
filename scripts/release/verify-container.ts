...
function main(): void {
  ...
  // The workspace install and build run on the CI runner; the image stage
  // consumes that proven tree through the prebuilt named context.
  const ecosystemIndex = dockerfile.indexOf('COPY --from=sdkwork-ecosystem')
  ...
  assert(ecosystemIndex !== -1, 'Dockerfile must copy the SDKWork ecosystem siblings')
  assert(prebuiltIndex > ecosystemIndex, 'Dockerfile must consume the prebuilt workspace after the sibling trees')
  assert(dshPackIndex > prebuiltIndex, 'Dockerfile must pack the dsh family after the prebuilt copy')
  ...
  assert(!/\bpnpm\s+run\s+build\b/.test(dockerInstructions), 'Dockerfile must not rebuild the workspace inside the image')
  assert(!/\bpnpm\s+install\b/.test(dockerInstructions), 'Dockerfile must not reinstall the workspace inside the image')
  ...
  assert(dockerfile.includes('native/system/scripts/pack-release.mjs /packs/landlock'), 'Dockerfile must use the complete Landlock release packer')
  assert(!dockerfile.includes('native/system/packages/entry pack'), 'Dockerfile must not pack only the Landlock entry package')
  ...
  const nativeJob = objectValue(workflowJobs['container-native'], 'release workflow container native job')
  ...
  assert(resolveJob['timeout-minutes'] === 5 && bundleJob['timeout-minutes'] === 15 && nativeJob['timeout-minutes'] === 15 && imageJob['timeout-minutes'] === 45 && releaseJob['timeout-minutes'] === 15, 'native release jobs must have bounded runtimes')
  ...
  assert(releaseWorkflow.includes('Assemble native Landlock artifacts') && releaseWorkflow.includes('chmod +x prebuilt/native/system/packages/linux-x64/bin/*'), 'container image context must contain executable binaries for both Landlock architectures')
  ...
  assert(releaseWorkflow.includes('Prepare retry-safe release state') && releaseWorkflow.includes('--method DELETE'), 'a retry must replace the release for the tag so rebuilt assets republish')
  assert(releaseWorkflow.includes('draft: true'), 'new uploads must stay draft until the exact asset verification passes')
