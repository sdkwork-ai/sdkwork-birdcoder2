...
function main(): void {
  ...
  assert(deploymentGuide.includes('minikube image load "dsh-container-image-${version}-linux-${arch}.tar"'), 'deployment guide must load the selected Release architecture into Minikube')
  ...
  assert(releaseWorkflow.includes('dsh-container-image-${RELEASE_VERSION}-linux-${RELEASE_ARCH}.tar.gz'), 'release workflow must name image archives by version and architecture')
  ...
  assert(releaseWorkflow.includes('dist/container/extracted/dsh-container-${RELEASE_VERSION}/docker-compose.yml'), 'release workflow must smoke Compose from the extracted deployment archive')
